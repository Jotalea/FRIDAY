...
std::string build_prompt(const std::string& query) {
    std::ostringstream p;
    p << "Generate ONLY a shell command for: " << query << "\n"
      << "Respond ONLY in this JSON format: {\"command\":\"[COMMAND]\",\"message\":\"[EXPLANATION]\"}\n"
      << "Use \"&&\" to chain commands; no multiline output. Do NOT use Markdown, since it WILL break the client-side parser. Be concise. If unsure, respond with {\"command\":\"null\"}.\n"
...
CommandResult ask_ai(std::string query) {
    ...
    if (res == CURLE_OK) {
        try {
            ...
            std::cerr << "API returned invalid JSON.\n";
        }
    } else {
...
void execute_command(const CommandResult& cmd) {
    ...
    if (!cmd.message.empty()) {
        std::cout << "Explanation: " << cmd.message << "\n";
    }
    std::cout << "\nRun? [y/N]: ";
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        std::cerr << "Usage: " << argv[0] << " <optionally, attach a @file>\n";
        return 1;
    }
    ...
    try {
        ...
        std::cerr << "Error creating config: " << e.what() << "\n";
        return 1;
    }
    ...
    std::cerr << "Could not generate a valid command.\n";
    return 1;
}
